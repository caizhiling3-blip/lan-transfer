...
6. 多任务串行队列（已完成）；
7. 后台通知和完成摘要（已完成）；
8. 安全完善与 Windows/macOS 双平台验收（实现与自动化已完成，实机矩阵待签字）。

## 阶段 8 安全验收结论

- 扫描端在 `lstat`、`realpath` 和再次 `lstat` 之间核对设备号、inode、类型和符号链接状态，上传前仍按已保存身份再次打开核对；
- manifest 文件和空目录按完整 NFC 相对路径严格升序，接收端拒绝无序、重复 ID、跨平台名称冲突、文件占用父路径和空目录填充分片；
- 每片实际内容不超过 96 KiB，累计条目、大小和部分 manifest 在全部分片到齐前持续受限，不能借 32 个分片突破 2 MiB 内存边界；
- 已终态 transferId 使用 24 小时、最多 2,000 项的有界集合防止新 messageId 重放旧 offer，协调器终态任务同样有容量上限；
- HTTP bearer token 继续绑定 connectionId、来源 IP、transferId、fileId、顺序、长度和期限，首次合法尝试后重复请求返回冲突；
- 断线、取消、超时、源文件变化和发布失败都会中止流并清理任务拥有的 staging/marker；真实回环集成测试覆盖活动上传中断。

自动化和当前构建机能证明协议、路径、清理及包结构，不能代替 NTFS/APFS 语义、Windows Defender、macOS 本地网络权限、真实防火墙和跨设备性能测试。实机状态以 [测试文档](testing.md) 的 1.2 签字矩阵为准。
