...
页面底部只有一个编辑器：用户可输入文字或链接、通过系统选择器添加一个或多个文件，或把文件拖入编辑器。文件先显示在待发送区，可逐个移除或清空；点击“发送”时先发送非空文字，再发起一个文件 offer。两种内容在时间线上仍是独立记录，因此任一部分失败都能给出准确状态，也不需要扩展第一版协议为复合消息。收到的文件 offer 直接在对应文件卡片内接受、改选目录或拒绝，不再显示与时间线分离的弹窗区。
