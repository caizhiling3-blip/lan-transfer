...
历史查询、统计和清理由主进程 `SessionHistory` 执行。renderer 只能提交经 schema 校验的筛选条件或 UUID 集合，`history.json` 文件大小由主进程从固定 store 路径读取；成功接收文件的历史 ID 与最终路径单独保存在 `history-locators.json`，清理摘要时同步删除。定位 IPC 只接受 history ID 并由主进程调用系统文件管理器，文件系统路径不会通过 IPC 暴露给页面。文字和链接可从历史页切换到传输时间线，并载入、滚动和高亮对应摘要。
