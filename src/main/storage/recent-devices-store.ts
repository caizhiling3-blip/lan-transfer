...
import { backupInvalidStoreFile, recentDevicesStoreSchema } from './store-schemas'
...
export class RecentDevicesStore {
  ...
  public constructor(directory: string) {
    backupInvalidStoreFile(directory, 'recent-devices', recentDevicesStoreSchema)
    this.store = new ElectronStore<RecentDevicesStoreData>({
      cwd: directory,
      name: 'recent-devices',
      defaults: { schemaVersion: 1, devices: [] },
    })
    recentDevicesStoreSchema.parse(this.store.store)
  }

  public add(device: DeviceInfo): void {
    const devices = this.list().filter((entry) => entry.device.deviceId !== device.deviceId)
    devices.unshift({ device, lastConnectedAt: Date.now() })
