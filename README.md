...
下一版本 v0.4.0 已启用协议 v3：设备使用 Ed25519/X25519 完成身份认证和首次验证码配对，WebSocket 控制消息与 HTTP 文件块均通过 AES-256-GCM 加密认证。文件和文件夹按 4 MiB 固定块上传，接收端只有在全部块认证且整文件 SHA-256 一致后才发布；支持手动暂停/继续、两分钟内可信设备自动重连，以及应用重启后从加密恢复记录继续缺失块。完整路线见 [v0.4.0 设计](docs/v0.4.0.md)。
