...
当前进度：1.2 文件夹传输阶段 3（清单 offer 与接收审批）。协议已升级到 v2；发送方把有界 manifest 分片经 WebSocket 发送，接收方只有在校验分片、摘要、数量、大小和跨平台路径后才显示整个文件夹的接受/拒绝入口。阶段 4 才会通过 HTTP 上传文件夹内容，当前接受后的任务停留在“已接受”。完整边界见 [文件夹传输设计](docs/folder-transfer.md)。
